...
if ( ( isset( $_POST[ "Toevoegen" ] ) ) && ( $_POST[ "Toevoegen" ] == "Toevoegen" ) ) {
	$insertSQL = sprintf( "INSERT INTO concert (Ensemble, Link, Olv, Mmv, ConcertTitel, Details, Datum, Tijd, Plaats, Prijsinfo, Reserveren, Digilink) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
		quote($_POST[ 'Ensemble' ]),
		quote($_POST[ 'Link' ]),
		quote($_POST[ 'Olv' ]),
		quote($_POST[ 'Mmv' ]),
		quote($_POST[ 'ConcertTitel' ]),
		quote($_POST[ 'Details' ]),
		quote($_POST[ 'Datum' ]),
		quote($_POST[ 'Tijd' ]),
		quote($_POST[ 'Plaats' ]),
		quote($_POST[ 'Prijsinfo' ]),
		htmlspecialchars_decode(quote($_POST[ 'Reserveren' ])),
		quote($_POST[ 'Digilink' ] ));
...
<body>
	<div class="w3-row w3-container">
			...
			<form id="vinden" method="post" action="<?php echo $_SERVER['../PHP_SELF']; ?>">
				...
				<ul id="navlist" class="w3-ul">
					...
					<?php foreach($zoek as $zoekitem) {
							...
							$timeType = IntlDateFormatter::NONE;//type of time formatting setting to none
							$formatter =new IntlDateFormatter($locale, $dateType, $timeType);
							$dateTime = new DateTime($zoekitem['Datum']);
							$datum = $formatter->format($dateTime); ?>
					<li>
		...
		<div id="main" class="w3-main w3-container" style="margin-left:260px;">
			<span class="w3-opennav w3-xlarge w3-hide-large" onclick="w3_open()">&#9776;</span>
			<form action="<?php echo $editFormAction; ?>" method="post" enctype="multipart/form-data" name="concert" id="concert">
				<div class="w3-margin-top w3-card-4" style="max-width: 1000px; float: left;">
					<table class="w3-table w3-striped w3-border" id="concertgegevens">
						...
						<tr valign="baseline">
							...
							<td><input type="text" name="Mmv" value="<?php echo stripslashes($concert['Mmv']); ?>" size="32"/>
							</td>
							<td align="right" nowrap>Reserveren:</td>
							<td><input type="text" name="Reserveren" value="<?php echo htmlspecialchars($concert['Reserveren']); ?>" size="40"/>
							</td>
						</tr>
						<tr valign="baseline">
							<td width="10%" align="right" nowrap>Concerttitel:</td>
							<td><input type="text" name="ConcertTitel" value="<?php echo stripslashes($concert['ConcertTitel']); ?>" size="32"/>
							</td>
				...
				<div id="digiflyer" class="w3-card-4 w3-margin-top" style="max-width: 200px; float: right;">
 					<?php if (isset($concert['Digilink']) AND $concert['Digilink'] != '') echo <<<EOT
						<img class="w3-image" style="max-width: 200px;" src="../Digiflyers/{$concert['Digilink']}" alt="Digiflyer {$concert['ConcertTitel']}"/> 
EOT;
					else echo '<p class="w3-margin w3-text-red">Geen afbeelding beschikbaar</p>';
					?>
 			</div>
			</form>
		</div>
